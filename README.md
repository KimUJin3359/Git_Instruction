...
- git fetch "name"
  - 경로의 주소(name)에 있는 상태를 branch로 받아옴
    - 원격 저장소의 최신 이력을 확인할 수 있음
    - 가져온 최시 커밋 이력은 이름 없는 브랜치로 로컬에 가져옴
  - 'FETCH_HEAD'의 이름으로 체크아웃 가능
    - 원격 저장소의 내용을 'master'에 통합하고 싶은 경우 'FETCH_HEAD' 브랜치를 merge or pull 실행

  | pull | fetch |
  | --- | --- |
  | 원격 저장소의 내용을 가져와 자동으로 병합 작업을 실행 | 원격 저장소의 내용을 확인만 하고 병합은 하고 싶지 않는 경우 사용 |
...
- git tag
  - 태그 목록 보기
  - -n 옵션 : 태그 목록과 주석 내용을 확인
- git tag -am "tag 주석" tag_name"
  - -a 옵션 : 주석이 달린 태그 추가
  - -m 옵션 : 태그의 주석내용 바로 입력
- git tag -d "tag_name"
  - 태그 
- 태그 : 커밋을 참조하기 쉽도록 알기 쉬운 이름을 붙이는 것
  - 일반 태그 : 이름만 붙일 수 있음 => 토픽 브랜치에서 주로 사용
  - 주석 태그 : 이름, 태그에 대한 설명, 서명, 이름, 이메일 등 포함 가능 => 릴리스 브랜치에서 주로 사용
---
...
### git 작업 취소
  ...
  | 모드명 | soft | mixed | hard |
  | --- | --- | --- | --- |
  | HEAD의 위치 | 변경함 | 변경함 | 변경함 |
  | 인덱스 | 변경 안함 | 변경함 | 변경함 |
  | 작업 트리 | 변경 안함 | 변경 안함 | 변경함 |
